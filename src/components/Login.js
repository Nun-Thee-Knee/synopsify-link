...
const Login = (props) => {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
    if (credentials['password'][0] !== credentials['confirmPassword'][0] && form !== "Login")
    {
      ...
      <div className='flex flex-col justify-start items-center h-[600px] w-[500px] bg-black bg-opacity-30 border-red-800 border-[1px] rounded-xl p-14'>
        ...
        <div id='form' className='mt-10 w-full'>
          <form onSubmit={handleSubmit} action="">
            <input onChange={handleChange} id="email" name="email" className='bg-black rounded text-white w-full border mb-10 mt-10 border-gray-300' type="email" placeholder="Enter your email" />
            <input onChange={handleChange} id="password" name="password" className='bg-black rounded text-white w-full border mb-10 border-gray-300' type="password" placeholder="Enter your password" />
            <input onChange={handleChange} id="confirmPassword" name="confirmPassword" className={`${form === "Login"?"hidden":""} bg-black text-white w-full border border-gray-300 rounded`} type="password" placeholder="Confirm password" />
            <p className='text-red-400'>{msg}</p>
            <center>
            <button className='hover:bg-red-800 rounded-2xl border-red-800 border-[1px] w-full text-white p-3 mt-10' type='submit'>
              <a href="/">{form}</a>
            </button>
            </center>
          </form>
